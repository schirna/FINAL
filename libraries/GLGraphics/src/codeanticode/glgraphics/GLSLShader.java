...
public class GLSLShader extends GLShader {
   ...
   * will generate. This method needs to be called after creating the geometry
   * shader and before linking the program.
   * 
   * @param inGeoPrim
   *          String
   * @param outGeoPrim
  ...
  public void setupGeometryShader(String inGeoPrim, String outGeoPrim,
    ...
    gl.glProgramParameteriEXT(programObject, GL.GL_GEOMETRY_INPUT_TYPE_EXT,
        inGeo);
    // Second, what type of primitive this shader will output.
    gl.glProgramParameteriEXT(programObject, GL.GL_GEOMETRY_OUTPUT_TYPE_EXT,
        outGeo);
    ...
    n = buf.get(0);
    if ((0 < maxNumOutVert) && (maxNumOutVert < n))
      n = maxNumOutVert;

    gl
        .glProgramParameteriEXT(programObject, GL.GL_GEOMETRY_VERTICES_OUT_EXT,
            n);
  }     
  ...
  /**
   * Sets the texture uniform with name to the given value 
   * 
   * @param name
   *          String
   * @param val
   *          int
   */  
  public void setTexUniform(String name, int val) {
    int loc = getUniformLocation(name);
    if (-1 < loc) gl.glUniform1iARB(loc, val); 
  }  
  ...
  private void attachVertexShader(String shaderSource, String file) {
    vertexShader = gl.glCreateShader(GL.GL_VERTEX_SHADER_ARB);
