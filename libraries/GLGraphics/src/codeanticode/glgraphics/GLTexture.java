...
public class GLTexture extends PImage implements PConstants, GLConstants {
  ...
  public void putBuffer(int[] intArray, int format, int type) {
    ...
    if (texTarget == GL.GL_TEXTURE_1D) {
      gl.glTexSubImage1D(texTarget, 0, 0, width, glFormat, glType, IntBuffer
          .wrap(convArray));
    } else {
      if (usingMipmaps)
        GLState.glu.gluBuild2DMipmaps(texTarget, texInternalFormat, width,
            height, glFormat, glType, IntBuffer.wrap(convArray));
      else
        gl.glTexSubImage2D(texTarget, 0, 0, 0, width, height, glFormat, glType,
            IntBuffer.wrap(convArray));
    }
