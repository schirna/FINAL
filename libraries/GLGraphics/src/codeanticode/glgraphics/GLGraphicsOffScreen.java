...
public class GLGraphicsOffScreen extends PGraphicsOpenGL implements GLConstants {
  ...
  protected int[] colorBufferMulti = null;
  protected int[] depthStencilBuffer = null;
  ...
  /**
   * Creates an instance of GLGraphicsOffScreen with specified size, with
   * mulisampling disabled.
   * 
   * @param parent
   *          PApplet
   * @param iwidth
  ...
  /**
   * Creates an instance of GLGraphicsOffScreen with specified size and
   * parameters for the destination texture. Multisampling is disabled.
   * 
   * @param parent
   *          PApplet
   * @param iwidth
  ...
  protected void finalize() throws Throwable {
    if (colorBufferMulti != null && colorBufferMulti[0] != 0) {
      gl.glDeleteRenderbuffersEXT(1, colorBufferMulti, 0);
    }

    if (depthStencilBuffer != null && depthStencilBuffer[0] != 0) {
      gl.glDeleteRenderbuffersEXT(1, depthStencilBuffer, 0);
    }
  ...
  /**
   * Returns the OpenGL capabilities of the drawable associated to this
   * renderer.
   * 
  ...
  /**
   * Creates a new context for canvas that is shared with the context of
   * GLGraphics.
   * 
   * @param GLCanvas
   *          canvas
  ...
  public void shareContext(GLCanvas canvas) {
    if (context != null)
      canvas.createContext(context);
  }

  /**
   * Creates a new context for drawable that is shared with the context of
   * GLGraphics.
   * 
   * @param GLDrawable
   *          drawable
  ...
  public void shareContext(GLDrawable drawable) {
    if (context != null)
      drawable.createContext(context);
  }
  ...
  /**
   * Any drawing operation with this rendered must be enclosed between a call to
  ...
  public void endDraw() {
    ...
    if (multisampleEnabled) {
      ...
      gl.glBindFramebufferEXT(GL.GL_READ_FRAMEBUFFER_EXT, multisampleFBO
          .getFramebufferID()); // source
      gl.glBindFramebufferEXT(GL.GL_DRAW_FRAMEBUFFER_EXT, FBO
          .getFramebufferID()); // dest
      gl.glBlitFramebufferEXT(0, 0, width, height, 0, 0, width, height,
  ...
  public void rotate(float angle) {
    if (glMode)
      gl.glRotatef(PApplet.degrees(angle), 0.0f, 0.0f, 1.0f);
    else
      super.rotate(angle);
  }
  ...
  public void rotateX(float angle) {
    if (glMode)
      gl.glRotatef(PApplet.degrees(angle), 1.0f, 0.0f, 0.0f);
    else
      super.rotateX(angle);
  }
  ...
  public void rotateY(float angle) {
    if (glMode)
      gl.glRotatef(PApplet.degrees(angle), 0.0f, 1.0f, 0.0f);
    else
      super.rotateY(angle);
  }
  ...
  public void lights() {
    if (glMode) {
       ...
       * directionalLight(colorModeX * 0.5f, colorModeY * 0.5f, colorModeZ *
       * 0.5f, 0, 0, -1);
       */
  ...
  private void glLightNoAmbient(int num) {
    gl.glLightfv(GL.GL_LIGHT0 + num, GL.GL_AMBIENT, zeroBufferGL, 0);
  }
  ...
  public void ortho(float left, float right, float bottom, float top,
    ...
    // Will call updateProjection in PGraphicsOpenGL, which copies the
    // projection matrix
    // to opengl
  ...
  protected void initFramebuffer() {
    ...
    if (multisampleEnabled) {
      ...
      gl.glBindRenderbufferEXT(GL.GL_RENDERBUFFER_EXT, colorBufferMulti[0]); // Binding
                                                                             // render
                                                                             // buffer
      gl.glRenderbufferStorageMultisampleEXT(GL.GL_RENDERBUFFER_EXT,
          multisampleLevel, GL.GL_RGBA8, width, height);

      // Creating handle for multisampled depth buffer.
      gl.glBindRenderbufferEXT(GL.GL_RENDERBUFFER_EXT, depthStencilBuffer[0]); // Binding
                                                                               // depth
                                                                               // buffer
      // Allocating space for multisampled depth buffer
      gl.glRenderbufferStorageMultisampleEXT(GL.GL_RENDERBUFFER_EXT,
          multisampleLevel, GL_DEPTH24_STENCIL8, width, height);
      ...
      gl.glFramebufferTexture2DEXT(GL.GL_FRAMEBUFFER_EXT,
          GL.GL_COLOR_ATTACHMENT0_EXT, GL.GL_TEXTURE_2D, colorTex
              .getTextureID(), 0);
      ...
      gl.glRenderbufferStorageEXT(GL.GL_RENDERBUFFER_EXT, GL_DEPTH24_STENCIL8,
          width, height);
      ...
      gl.glFramebufferTexture2DEXT(GL.GL_FRAMEBUFFER_EXT,
          GL.GL_COLOR_ATTACHMENT0_EXT, colorTex.getTextureTarget(), colorTex
              .getTextureID(), 0);
      gl.glFramebufferRenderbufferEXT(GL.GL_FRAMEBUFFER_EXT,
          GL.GL_DEPTH_ATTACHMENT_EXT, GL.GL_RENDERBUFFER_EXT,
          depthStencilBuffer[0]);
