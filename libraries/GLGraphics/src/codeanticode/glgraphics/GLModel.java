...
public class GLModel implements PConstants, GLConstants {
  ...
  protected void finalize() throws Throwable {
    if (vertCoordsVBO[0] != 0) {
      gl.glDeleteBuffers(1, vertCoordsVBO, 0);
    }

    if (colorsVBO != null && colorsVBO[0] != 0) {
      gl.glDeleteBuffers(1, colorsVBO, 0);
    }

    if (normCoordsVBO != null && normCoordsVBO[0] != 0) {
      gl.glDeleteBuffers(1, normCoordsVBO, 0);
    }

    if (texCoordsVBO != null && texCoordsVBO[0] != 0) {
      gl.glDeleteBuffers(numTextures, texCoordsVBO, 0);
    }

    if (attribVBO != null && attribVBO[0] != 0) {
      gl.glDeleteBuffers(numAttributes, attribVBO, 0);
    }
  ...
  public void beginUpdateVertices() {
    gl.glBindBufferARB(GL.GL_ARRAY_BUFFER_ARB, vertCoordsVBO[0]);
    vertices = gl.glMapBufferARB(GL.GL_ARRAY_BUFFER_ARB, GL.GL_WRITE_ONLY)
        .asFloatBuffer();
  }
  ...
  public void beginUpdateColors() {
    gl.glBindBufferARB(GL.GL_ARRAY_BUFFER_ARB, colorsVBO[0]);
    colors = gl.glMapBufferARB(GL.GL_ARRAY_BUFFER_ARB, GL.GL_WRITE_ONLY)
        .asFloatBuffer();
  }
  ...
  public void beginUpdateTexCoords(int n) {
    ...
    texCoords = gl.glMapBufferARB(GL.GL_ARRAY_BUFFER_ARB, GL.GL_WRITE_ONLY)
        .asFloatBuffer();
  }
  ...
  public void beginUpdateNormals() {
    gl.glBindBufferARB(GL.GL_ARRAY_BUFFER_ARB, normCoordsVBO[0]);
    normals = gl.glMapBufferARB(GL.GL_ARRAY_BUFFER_ARB, GL.GL_WRITE_ONLY)
        .asFloatBuffer();
  }
  ...
  public void beginUpdateAttributes(int n) {
    ...
    attributes = gl.glMapBufferARB(GL.GL_ARRAY_BUFFER_ARB, GL.GL_WRITE_ONLY)
        .asFloatBuffer();
   ...
   * @param String
   *          filename
   * @return ArrayList<PVector> int len
   */
  ...
  public void render(int first, int last, GLModelEffect effect) {
    ...
    if (usingPointSprites)
      gl.glPointSize(PApplet.min(pointSize, maxPointSize));
    else
    ...
    if (texCoordsVBO != null) {
      ...
      if (usingPointSprites) {
        ...
        gl.glPointParameterfvARB(GL.GL_POINT_DISTANCE_ATTENUATION_ARB,
            ptDistAtt, 0);

        // The alpha of a point is calculated to allow the fading of points
        // instead of shrinking them past a defined threshold size. The
        // threshold
        // is defined by GL_POINT_FADE_THRESHOLD_SIZE_ARB and is not clamped to
        // the minimum and maximum point sizes.
        gl.glPointParameterfARB(GL.GL_POINT_FADE_THRESHOLD_SIZE_ARB,
            spriteFadeSize);
